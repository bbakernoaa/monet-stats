...
obs_data = xr.DataArray(np.random.rand(100, 100), coords=[lat, lon], dims=["lat", "lon"], name="obs").chunk(
    {"lat": 50, "lon": 50}
)
...
stdo_map.plot(ax=ax, transform=ccrs.PlateCarree(), cmap="viridis", cbar_kwargs={"label": "STDO"})
ax.coastlines()
...
interactive_plot = stdo_map.hvplot.quadmesh(
    x="lon", y="lat", geo=True, rasterize=True, cmap="viridis", title="STDO (Interactive)"
)
